...
def MatGen2(SpeedsArr,func,func2,n):
    ...
    supsum=np.sum(np.square(SpeedsArr))
    for i in range(n**dim):
        ...
        for j in range(dim):
            indicies[j]=tmp%n
            tmp=tmp//n
        SpeedArr=SpeedsArr[func(indicies[0],indicies[1],n)][func2(indicies[0],indicies[1],n)]
        unloop=np.sum(np.square(SpeedArr))/supsum
        top=2*np.sum(np.square(SpeedArr))
        for j in range(dim):
            #ret[AddInd(indicies,j,-1,n)][i]+=SpeedArr[j]**2/top*unloop
            #ret[AddInd(indicies,j,1,n)][i]+=SpeedArr[j]**2/top*unloop
            ret[AddInd(indicies,j,-1,n)][i]+=(0.5-math.atan(SpeedArr[j]/SpeedArr[1-j])*1/math.pi)*unloop
            ret[AddInd(indicies,j,1,n)][i]+=(0.5-math.atan(SpeedArr[j]/SpeedArr[1-j])*1/math.pi)*unloop
        ret[i][i]+=(1-unloop)
    return ret
def Graph(ei,n):
